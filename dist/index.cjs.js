...
const AuthProvider = ({ children, refreshEndpoint, refreshStrategy = "storage", storage, fetcher = fetch, mfaEndpoint, }) => {
    ...
    const refreshTimer = React.useRef(undefined);
...
const ProtectedRoute = ({ children, roles, permissions, redirectTo = "/login", forbiddenTo = "/403", }) => {
    ...
    if (!isAuthenticated) {
        return React.createElement(reactRouterDom.Navigate, { to: redirectTo, replace: true });
    }
    if (!canAccess({ roles, permissions })) {
        return React.createElement(reactRouterDom.Navigate, { to: forbiddenTo, replace: true });
    }
    return children;
};
...
const withRole = (WrappedComponent, options) => {
    const WithRole = (props) => {
        ...
        if (!isAuthenticated) {
            return React.createElement(reactRouterDom.Navigate, { to: loginPath, replace: true });
        }
